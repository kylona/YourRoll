...
class GiftedChat extends React.Component {
    ...
    static insert(currentMessages = [], messages, sortBy,  inverted = true) {
        if (!Array.isArray(messages)) {
            messages = [messages];
        }
        for (m in messages) {
            let insertPoint = currentMessages.findIndex((element) => {
              let isGreater = element[sortBy] > messages[m][sortBy]
              return inverted ? !isGreater : isGreater
            })
            currentMessages.splice(insertPoint, 0, messages[m])
        }
